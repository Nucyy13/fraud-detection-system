...
def userLogout(request):
    try:
        # Delete the 'username' session variable
        del request.session['username']
    except KeyError:
        logger.exception("Failed to delete 'username' session variable")

    try:
        # Call the logout function from django.contrib.auth
        logout(request)
    except Exception as e:
        logger.exception("Logout error: {}".format(e))

    # Redirect to a specific URL after logout, in this case, the home page '/'
    return HttpResponseRedirect('/')

def cardverifier(request):
    return render(request, 'homeApp/cardverifier.html')
...
def predict_csv_single(request,id):
    if request.method == 'POST':
        try:
            ...
            status = "Non-Fraudulent Transaction" if prediction[0] == 0 else "Fraudulent Transaction"
            ...
            return HttpResponseRedirect(reverse('add_files_single', args=[id]))
